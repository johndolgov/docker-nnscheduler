import numpy as np
from src.env.entities import volume_to_transfer, estimate_comp_times
...
class Context:
    ...
    def construct_state(self):
        ...
        inputs = np.zeros((self.state_n, len(self.nodes)))
        self.candidates = ready_tasks

        for i, ready_task in enumerate(ready_tasks):
            n2 = self.calc_n2(ready_task)
            n1 = self.calc_n1(ready_task)

            H1 = n1 / np.sum(n1)
            H2 = n2 / np.sum(n2)

            H1 = np.clip(H1, 1e-10, None)
            H2 = np.clip(H2, 1e-10, None)

            E1 = -np.log(len(self.nodes))**(-1) * np.sum(H1 * np.log(H1))
            E2 = -np.log(len(self.nodes))**(-1) * np.sum(H2 * np.log(H2))
    ...
    def calc_n2(self, task):
        m = np.zeros(len(self.nodes))
        for i, node in enumerate(self.nodes):
            rt = self.comp_times[task, i]
            m[i] = rt
        k = m - np.min(m)
        j = np.max(m) - np.min(m)
        n2 = 1 - (k / j)
    ...
    def calc_n1(self, task):
        l = np.zeros(len(self.nodes))
        for i, node in enumerate(self.nodes):
            rt = self.comp_times[task, i]
            node_sched = self.schedule[i]
            if len(node_sched) > 0:
                node_last_time = node_sched[-1].end_time
            else:
                node_last_time = 0.0
            l[i] = rt + node_last_time
        k = l - np.min(l)
        j = np.max(l) - np.min(l)
        n1 = 1 - (k / j)
